...
$iron_features_post_types = array('event', 'album', 'video');
...
function post_type_init() {
   ...
   ========================================================================== */
    ...
    $artists_args['supports'] = array('title', 'editor', 'revisions');


    $artists_args['menu_icon'] = 'dashicons-groups';

    $artists_args['rewrite'] = array('slug'=> esc_html__('artist', 'iron-music') );
   ...
   ========================================================================== */
    ...
    $video_args['rewrite'] = array('slug'=> esc_html__('videos', 'iron-music'));
...
function iron_events_where ( $where = '' ){
    ...
    // var_dump( $iron_features_query->query_vars['filter'] );
    ...
    if( $iron_features_query->post_type != 'event' )
        return $where;
...
function iron_music_manage_video_columns ($columns)
{
    ...
    $columns['date'] = __('Published', 'croma'); // Renamed date column
...
function iron_music_manage_video_custom_column ($column, $post_id)
{
    switch ($column)
    {
        case 'icon':
            $att_title = _draft_or_post_title();
?>
                <a href="<?php echo esc_url(get_edit_post_link( $post_id, true )); ?>" title="<?php echo esc_attr( sprintf( __('Edit &#8220;%s&#8221;', 'croma'), $att_title ) ); ?>"><?php
                    if ( $thumb = get_the_post_thumbnail( $post_id, array(80, 60) ) )
                        echo $thumb;
                    else
                        echo '<img width="46" height="60" src="' . wp_mime_type_icon('image/jpeg') . '" alt="">';
...
/* Video Categories (video-categories)
   ========================================================================== */
    ...
    $labels = array(
          'name'          => esc_html_x('Video Categories',     'Taxonomy : name',          'croma')
        , 'all_items'     => esc_html_x('All Categories',       'Taxonomy : all_items',     'croma')
        , 'singular_name' => esc_html_x('Category',             'Taxonomy : singular_name', 'croma')
        , 'add_new_item'  => esc_html_x('Add New Category',     'Taxonomy : add_new_item',  'croma')
        , 'not_found'     => esc_html_x('No categories found.', 'Taxonomy : not_found',     'croma')
    );
...
/* Discography Categories (discography-categories)
   ========================================================================== */
    ...
    $labels = array(
          'name'          => esc_html_x('Discography Categories', 'Taxonomy : name',          'croma')
        , 'all_items'     => esc_html_x('All Categories',       'Taxonomy : all_items',     'croma')
        , 'singular_name' => esc_html_x('Category',             'Taxonomy : singular_name', 'croma')
        , 'add_new_item'  => esc_html_x('Add New Category',     'Taxonomy : add_new_item',  'croma')
        , 'not_found'     => esc_html_x('No categories found.', 'Taxonomy : not_found',     'croma')
...
/* Artists Categories (photo-album-categories)
   ========================================================================== */
    ...
    $labels = array(
          'name'          => esc_html_x('Artists Categories',   'Taxonomy : name',          'croma')
        , 'all_items'     => esc_html_x('All Categories',       'Taxonomy : all_items',     'croma')
        , 'singular_name' => esc_html_x('Category',             'Taxonomy : singular_name', 'croma')
        , 'add_new_item'  => esc_html_x('Add New Category',     'Taxonomy : add_new_item',  'croma')
        , 'not_found'     => esc_html_x('No categories found.', 'Taxonomy : not_found',     'croma')
...
/* Events Categories (photo-album-categories)
   ========================================================================== */
    ...
    $labels = array(
          'name'          => esc_html_x('Events Categories',   'Taxonomy : name',          'croma')
        , 'all_items'     => esc_html_x('All Categories',       'Taxonomy : all_items',     'croma')
        , 'singular_name' => esc_html_x('Category',             'Taxonomy : singular_name', 'croma')
        , 'add_new_item'  => esc_html_x('Add New Category',     'Taxonomy : add_new_item',  'croma')
        , 'not_found'     => esc_html_x('No categories found.', 'Taxonomy : not_found',     'croma')
