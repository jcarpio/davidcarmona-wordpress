		...
		function render ()
		{
			...
			foreach ( $groups as $g_id => $group )
			{
				...
				if ( isset( $group[ $group_key ] ) && ! empty( $group[ $group_key ] ) )
					...
					$group_title = esc_html__('New', 'croma') . ' ' . $group_name;
				...
				echo '</table>';
				echo '<a href="javascript:void(0);" class="button deletion redux-groups-remove">' . esc_html__('Delete', 'croma') . ' ' . $group_name . '</a>';
				echo '</div>';
				echo '</li>';
			...
			echo '</ul>';
			echo '<a href="javascript:void(0);" data-next-id="" class="button redux-groups-add button-primary" rel-id="' . $this->field['id'] . '-ul" rel-name="' . $this->args['opt_name'] . '[' . $this->field['id'] . '][' . $group_key . '][]">' . esc_html__('Add', 'croma') . ' ' . $group_name . '</a><br/>';
			echo '</div>';
