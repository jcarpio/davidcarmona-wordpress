    ...
    class Redux_Options {
        ...
        */
        function __construct($sections = array(), $args = array(), $extra_tabs = array()) {
            ...
            $defaults['menu_title'] = esc_html__('Options', 'croma');
            $defaults['page_icon'] = 'fa fa-themes';
            $defaults['page_title'] = esc_html__('Options', 'croma');
            ...
            $defaults['footer_credit'] = wp_kses( sprintf(__('<span id="footer-thankyou">Options panel created using <a href="%s" target="_blank">Redux Framework</a> v%s</span>',
             'croma'), esc_url($this->framework_url), $this->framework_version),iron_get_allowed_html());
        ...
        */
        function _enqueue() {
            ...
            wp_localize_script('redux-opts-js', 'redux_opts', array('reset_confirm' => esc_html__('Are you sure? You will lose all custom values.', 'croma'), 'opt_name' => $this->args['opt_name']));
        ...
        function _options_page_html() {
            ...
            if(isset($_GET['settings-updated']) && $_GET['settings-updated'] == 'true' && get_transient('redux-opts-saved') == '1') {
                if(isset($this->options['imported']) && $this->options['imported'] == 1) {
                    echo '<div id="redux-opts-imported"><strong>' . apply_filters('redux-opts-imported-text-' . $this->args['opt_name'], esc_html__('Settings Imported!', 'croma')) . '</strong></div>';
                } else {
                    echo '<div id="redux-opts-save"><strong>' . apply_filters('redux-opts-saved-text-' . $this->args['opt_name'], esc_html__('Settings Saved!', 'croma')).'</strong></div>';
                }
                delete_transient('redux-opts-saved');
            }

            echo '<div id="redux-opts-save-warn"><strong>' . apply_filters('redux-opts-changed-text-' . $this->args['opt_name'], esc_html__('Settings have changed, you should save them!', 'croma')) . '</strong></div>';
            echo '<div id="redux-opts-field-errors"><strong><span></span> ' . esc_html__('error(s) were found!', 'croma') . '</strong></div>';
            echo '<div id="redux-opts-field-warnings"><strong><span></span> ' . esc_html__('warning(s) were found!', 'croma').'</strong></div>';
            ...
            if(true === $this->args['show_import_export']) {
                ...
                echo '<a href="javascript:void(0);" id="import_export_default_section_group_li_a" class="redux-opts-group-tab-link-a" data-rel="import_export_default">' . $icon . ' <span>' . esc_html__('Import / Export', 'croma') . '</span></a>';
                echo '</li>';
            ...
            if(true === $this->args['dev_mode']) {
                ...
                echo '<a href="javascript:void(0);" id="dev_mode_default_section_group_li_a" class="redux-opts-group-tab-link-a custom-tab" data-rel="dev_mode_default">' . $icon . ' <span>' . esc_html__('Dev Mode Info', 'croma') . '</span></a>';
                echo '</li>';
            }
            ...
            if(true === $this->args['show_import_export']) {
                ...
                echo '<h3>' . esc_html__('Import / Export Options', 'croma') . '</h3>';
                echo '<h4>' . esc_html__('Import Options', 'croma') . '</h4>';
                echo '<p><a href="javascript:void(0);" id="redux-opts-import-code-button" class="button-secondary">' . esc_html__('Import from file', 'croma') . '</a> <a href="javascript:void(0);" id="redux-opts-import-link-button" class="button-secondary">' . esc_html__('Import from URL', 'croma') . '</a></p>';

                echo '<div id="redux-opts-import-code-wrapper">';

                echo '<div class="redux-opts-section-desc">';
                echo '<p class="description" id="import-code-description">' . apply_filters('redux-opts-import-file-description', esc_html__('Input your backup file below and hit Import to restore your sites options from a backup.', 'croma')) . '</p>';
                echo '</div>';
                ...
                echo '<div class="redux-opts-section-desc">';
                echo '<p class="description" id="import-link-description">' . apply_filters('redux-opts-import-link-description', esc_html__('Input the URL to another sites options set and hit Import to load the options from that site.', 'croma')) . '</p>';
                echo '</div>';
                ...
                echo '<p id="redux-opts-import-action"><input type="submit" id="redux-opts-import" name="' . $this->args['opt_name'] . '[import]" class="button-primary" value="' . esc_html__('Import', 'croma') . '"> <span>' . apply_filters('redux-opts-import-warning', esc_html__('WARNING! This will overwrite any existing options, please proceed with caution!', 'croma')) . '</span></p>';
                echo '<div id="import_divide"></div>';

                echo '<h4>' . esc_html__('Export Options', 'croma') . '</h4>';
                echo '<div class="redux-opts-section-desc">';
                echo '<p class="description">' . apply_filters('redux-opts-backup-description', esc_html__('Here you can copy/download your current option settings. Keep this safe as you can use it as a backup should anything go wrong, or you can use it to restore your settings on this site (or any other site).', 'croma')).'</p>';
                echo '</div>';
            ...
            if(true === $this->args['dev_mode']) {
                ...
                echo '<h3>' . esc_html__('Dev Mode Info', 'croma') . '</h3>';
                echo '<div class="redux-opts-section-desc">';
            ...
            submit_button('', 'primary', '', false);
            echo ' &nbsp; ';
            submit_button(esc_html__('Reset to Defaults', 'croma'), 'secondary', $this->args['opt_name'] . '[defaults]', false);
            echo '<div class="clear"></div><!--clearfix-->';
            echo '</div>';
