...
class Iron_Croma_Setup{
	...
	public static function execute(){
		...
		add_action( 'init', array('iron_croma_setup', 'dynamicAssets'));
		...
		add_action( 'template_redirect', array('iron_croma_setup', 'redirectAlbumUrl'));
		...
		add_action(	'updated_option', array( 'iron_croma_setup', 'iron_croma_write_dynamic_assets' ), 10, 3 );
	...
	public static function redirectAlbumUrl () {
	...
	public static function registerRequiredPlugins (){
		$plugins = array(
			array(
				...
				'source'			=> 'http://croma.irontemplates.com/iron-plugins/croma-music-2.4.1.zip'
			),
			array(
				...
				'required' 	=> false,
				'version'	=> '1.0',
				'source'	=> 'http://croma.irontemplates.com/iron-plugins/iron-demo-importer.zip'
			),
			array(
				'name'     => 'Contact Form 7',
				'slug'     => 'contact-form-7',
	...
	public static function enqueueStyles() {
		...
		wp_enqueue_style('iron-font-awesome', get_template_directory_uri().'/css/font-awesome.min.css', false, '', 'all' );
		...
		wp_add_inline_style( 'custom-styles', self::iron_croma_inline_dynamic_assets() );
	...
	public static function enqueueScripts() {
		...
		wp_enqueue_script('iron-main', get_template_directory_uri().'/js/main.js', array('jquery', 'iron-plugins', 'barba','underscore','wpb_composer_front_js'), null, true);
		...
		wp_localize_script('iron-main', 'iron_vars', array(
				...
				'enable_nice_scroll' => Iron_Croma::getOption('enable_nice_scroll', null, '0') == "0" ? false : true,
				...
				'menu' => array(
					...
					'classic_menu_over_content' => ( Iron_Croma::getField('classic_menu_over_content', get_the_ID() )  == '1' || get_post_type( $post ) == 'album')? '1': '0',
				...
				'logo' => array(
					...
					'use_alternative_logo'=> (bool) Iron_Croma::getField('alternative_logo', get_the_ID() ),
	...
	public static function dynamicAssets() {
		if(is_admin() || Iron_Croma::isLoginPage()) return -1;

		if(!empty($_GET["load"])) {
			if($_GET["load"] == 'custom-style.css' || $_GET["load"] == 'custom-post-style.css') {
				include_once(get_template_directory().'/css/custom-style.php');
				exit;
			}
		}
	}
	...
	*/
	...
	private static function iron_croma_inline_dynamic_assets(){
		global $post;

		ob_start();

		include( get_template_directory() .'/css/custom-style-post.php');
		// put content in data
		$data = ob_get_clean();
	...
	public function iron_croma_write_dynamic_assets( $option, $old_value, $value ){
		...
		if ( $option == 'croma' && array_key_exists('external_css', $value ) && $value['external_css'] == '1'){
			...
			include( get_template_directory() .'/css/custom-style.php');
			// put content in data
			$data = ob_get_clean();
