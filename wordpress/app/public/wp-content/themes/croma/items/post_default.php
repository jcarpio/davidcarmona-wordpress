			...
			<div class="text-box<?php if(!has_post_thumbnail()){ echo " empty"; }?>">
				...
				<div class="classic-meta">
					...
					<?php if ((bool)Iron_Croma::getOption('show_post_author', null, true)  &&  get_the_author() != '' ): ?>
						<a class="meta-author-link" href="<?php echo get_author_posts_url(get_the_author_meta('ID')); ?>"><?php echo esc_html__('by', 'croma'); ?> <?php the_author(); ?></a>
					<?php endif ?>
					<?php
						...
						echo '<div class="post-categories"><i class="fa fa-folder-open-o"></i> '.$iron_croma_categories_list.' </div> ';

						$iron_croma_tag_list = get_the_tag_list('',', ');
						if( !empty($tag_list) && (bool)Iron_Croma::getOption('show_post_tags', null, true) )
						echo '<div class="post-tags"> <i class="fa fa-tag"></i> '.$iron_croma_tag_list.'</div>';
